...
public:
  PIDMazeSolver(int scene_number, const rclcpp::NodeOptions &options)
    ...
    min_vx_         = this->declare_parameter<float>("min_vx",         0.07f);
...
private:
  ...
  float target_x_ = 0.0f, target_y_ = 0.0f; // world-frame goal, set at activation
  ...
  // Wall-parallel detection: nee_/see_ → LEFT side, nww_/sww_ → RIGHT side
  float nee_ = 9.9f, see_ = 9.9f;
  ...
  // Body corner indices (centre of each ±20° band)
  int idx_cfl_, idx_cfr_, idx_cbl_, idx_cbr_;
  // Back arc boundary indices (CBL → 180° → CBR)
  int idx_back_arc_l_, idx_back_arc_r_;
  int band_half_ = 5;
  ...
  void updateLaser(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
    ...
    nee_ = avg(idx_nee_); // left-side front
    see_ = avg(idx_see_); // left-side back
    nww_ = avg(idx_nww_); // right-side front
    sww_ = avg(idx_sww_); // right-side back

    // Wheel corner readings — ±20° band around each wheel centre
    // 20° in indices ≈ 20 / (angle_inc_deg)
    // Corner readings — ±20° band around each body corner
    int corner_band = static_cast<int>(20.0f / (angle_inc_ * 180.0f / M_PI));
    corner_band = std::max(corner_band, 3);
    auto avgWide = [&](int center, int half) {
      float sum = 0.0f;
      int cnt = 0;
      for (int i = std::max(0, center - half);
           i <= std::min(num_rays_ - 1, center + half); i++) {
        if (std::isfinite(r[i]) && r[i] > 0.01f && r[i] < rmax)
          sum += r[i], cnt++;
      }
      return cnt > 0 ? sum / cnt : 9.9f;
    };
    corner_fl_ = avgWide(idx_cfl_, corner_band);
    corner_fr_ = avgWide(idx_cfr_, corner_band);
    ...
    // we average from idx_back_arc_l_ to num_rays_-1, then 0 to idx_back_arc_r_
    {
      ...
      if (idx_back_arc_l_ <= idx_back_arc_r_) {
        // no wrap needed (shouldn't happen for 145°/-145° but guard anyway)
        accum(idx_back_arc_l_, idx_back_arc_r_);
      } else {
        // wraps: from CBL index to end, then from 0 to CBR index
        accum(idx_back_arc_l_, num_rays_ - 1);
        accum(0, idx_back_arc_r_);
      }
      back_arc_ = cnt > 0 ? sum / cnt : 9.9f;
    }
  }
  ...
  void controlLoop() {
    ...
    if (!goal_active_) {
      ...
      // Rotate goal dx/dy from robot-local frame → odom world frame.
      // Waypoints are written as "forward/right relative to the robot's
      // current heading" so dx=1.0 always means 1m in the direction the
      // robot is physically facing, regardless of odom orientation.
      float cy = std::cos(corrected_yaw);
      float sy = std::sin(corrected_yaw);
      target_x_ = start_x_ + cy * goal.x - sy * goal.y;
      target_y_ = start_y_ + sy * goal.x + cy * goal.y;
    ...
    if (phase_ == Phase::MOVING) {
      ...
      // err_lat≈0 even while the robot yaws, so vy never runs away.
      ...
      if (back_arc_ < back_nudge_thresh_) {
        vx += nudge_gain_ * 0.5f;
        RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 1000,
                             "[MOVE] Back arc nudge back_arc_=%.3f", back_arc_);
      }

      // ── Body corner nudge ─────────────────────────────────────────────
      // Only active when far enough from goal to avoid fighting the PID
      // near corner waypoints or just after leaving one.
      // Each corner triggers a very small nudge (0.5× nudge_gain_) away from
      // the obstacle — front corners push back, rear corners push forward,
      // left corners push right, right corners push left.
      if (dist > goal_tolerance_ * 3.0f) {
